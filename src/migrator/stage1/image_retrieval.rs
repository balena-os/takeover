...
pub(crate) fn download_image(
    ...
    let api_key = balena_cfg.get_api_key()
        .context(upstream_context!("Failed to retrieve api-key from config.json - unable to retrieve os-image"))?;

    let api_endpoint = balena_cfg.get_api_endpoint()
        .context(upstream_context!("Failed to retrieve api-endpoint from config.json - unable to retrieve os-image"))?;
    ...
    let version = match version {
        "latest" => {
            ...
            Version::parse(&versions.latest)
                .context(upstream_context!(&format!("Failed to parse version from '{}'", versions.latest)))?
        }
        "default" => {
